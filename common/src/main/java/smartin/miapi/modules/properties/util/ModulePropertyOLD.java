 ...
 */
public interface ModulePropertyOLD {
     ...
     * if any clientOnly stuff needs implementing {@link ModulePropertyOLD#load(String, JsonElement, boolean)}
     *
    ...
    static Map<ModulePropertyOLD, JsonElement> mergeList(Map<ModulePropertyOLD, JsonElement> old, Map<ModulePropertyOLD, JsonElement> toMerge, MergeType type) {
        Map<ModulePropertyOLD, JsonElement> mergedMap = new HashMap<>(old);
    ...
    default JsonElement getJsonElement(ModuleInstance moduleInstance) {
        return moduleInstance.getOldProperties().get(this);
    }
    ...
    default Map<ModuleInstance, JsonElement> nonNullJsonElements(ItemStack itemStack) {
        ...
        ItemModule.getModules(itemStack).allSubModules().forEach(moduleInstance -> {
            if (moduleInstance.getOldProperties().containsKey(this)) {
                maps.put(moduleInstance, moduleInstance.getOldProperties().get(this));
            }
        });
        return maps;
    }
    ...
    default Tuple<ModuleInstance, JsonElement> highestPriorityJsonElement(ItemStack itemStack) {
        ...
        for (ModuleInstance instance : ItemModule.getModules(itemStack).allSubModules()) {
            if (instance.getOldProperties().containsKey(this)) {
                moduleInstance = instance;
                element = instance.getOldProperties().get(this);
            }
        }
        if (moduleInstance == null) {
            return null;
