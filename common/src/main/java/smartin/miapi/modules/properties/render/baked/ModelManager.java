...
public class ModelManager {
    ...
    protected static BlockModel loadModelFromFilePath(String filePath2) throws FileNotFoundException {
        ...
        String finalFilePath = filePath2;
        holder.modelMetadata().modelSources().forEach(path->{
            try {
                loadModelFromFilePath(path);
            } catch (FileNotFoundException e) {
                Miapi.LOGGER.warn("could not load additional modelsources for model " + finalFilePath + " from path "+path);
            }
        });
