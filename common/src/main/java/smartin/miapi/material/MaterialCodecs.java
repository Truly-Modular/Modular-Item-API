...
public final class MaterialCodecs {
     ...
     */
    private static final Codec<Material> SIMPLE =
     ...
     */
    private static final Codec<Material> COMPLEX =
            Codec.STRING
                    .dispatch(
                            "type",
                            material -> material.getID().toString(),
                            id -> {
                                ...
                                return base.codec()
                                        .orElseThrow(() ->
                                                new IllegalStateException("Material " + id + " does not define a codec"));
                            }
                    );

    private static final Codec<Material> DECODE = Codec.withAlternative(SIMPLE, COMPLEX);

    public static final Codec<Material> MATERIAL_CODEC =
            Codec.of(
                    // Encoder
                    new Encoder<>() {
                        @Override
